...
def _traduzir_em_background(instance_pk, model_class, campos):
    """
    Roda a tradução em uma thread separada para não bloquear o save().
    `campos` é uma lista de (attr_pt, attr_lang, lang_dest).
    """
    def _run():
        try:
...
class Hotel(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Se o texto principal mudou, limpa as traduções antigas
        if self.pk:
            try:
                original = Hotel.objects.get(pk=self.pk)
                if (
                    original.titulo_hero != self.titulo_hero or
        ...
        # Traduz em background sem bloquear o save()
        campos = [
...
class PasseioAgenda(models.Model):
    ...
    def vagas_disponiveis(self):
        """Retorna vagas reais descontando reservas confirmadas."""
        reservadas = self.reservas.filter(
            status__in=[Reserva.Status.PENDENTE, Reserva.Status.CONFIRMADA]
        ).aggregate(total=models.Sum('qtd_pessoas'))['total'] or 0
...
class Reserva(models.Model):
    ...
    passeio_agenda = models.ForeignKey(
        PasseioAgenda, on_delete=models.CASCADE, related_name='reservas'
    )
    ...
    def clean(self):
        """Valida se há vagas disponíveis antes de salvar."""
        if self.passeio_agenda and self.qtd_pessoas:
            agenda = self.passeio_agenda
            disponiveis = agenda.vagas_disponiveis
            # Ao editar, desconta as próprias pessoas já reservadas
            if self.pk:
                disponiveis += self.qtd_pessoas
            if self.qtd_pessoas > disponiveis:
                raise ValidationError(
                    f"Vagas insuficientes. Disponíveis: {disponiveis}."
                )
    ...
    def __str__(self):
        return f"{self.nome_cliente} — {self.passeio_agenda} ({self.status})"
...
def criar_ou_atualizar_perfil(sender, instance, created, **kwargs):
    """Cria o Perfil na criação do User. Usa get_or_create para evitar IntegrityError."""
    if created:
...
class Hero(models.Model):
    # CORRIGIDO: era CharField(hotel_slug), agora é FK real para garantir integridade
    hotel = models.ForeignKey(
        Hotel, on_delete=models.CASCADE, related_name='heroes', null=True, blank=True
    )
