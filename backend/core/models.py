...
def get_media_url(field):
    ...
    Funciona com Cloudinary (django-cloudinary-storage) e armazenamento local.
    """
    if not field:
        return None
    # Se já for string (url direta), retorna como está
...
def api_passeios(request, hotel_slug, passeio_id=None):
    ...
    if request.method == "POST":
        try:
            ...
            passeio.preco_sob_consulta = request.POST.get("preco_sob_consulta") in ("true", "True", "1")
            passeio.preco_por_pessoa   = request.POST.get("preco_por_pessoa") in ("true", "True", "1")
