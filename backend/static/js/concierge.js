...
wpp_msg: function(n,p,q,d,h){var m=‘Olá! Me chamo *’+n+’* e tenho interesse no passeio *’+p+’* para *’+q+’ pessoa(s)*’;if(d)m+=’, na data *’+d+’*’;if(h)m+=’ às *’+h+’*’;m+=’. Poderia confirmar a disponibilidade?’;return m;},
...
lbl_nome: ‘Nombre completo’, lbl_tel: ‘Teléfono’, lbl_qtd: ‘Número de personas’,
...
wpp_msg: function(n,p,q,d,h){var m=’¡Hola! Me llamo *’+n+’* y estoy interesado/a en el paseo *’+p+’* para *’+q+’ persona(s)*’;if(d)m+=’ en la fecha *’+d+’*’;if(h)m+=’ a las *’+h+’*’;m+=’. ¿Podría confirmar disponibilidad?’;return m;},
...
fr: {
btn: ‘Voir détails’, vazio: ‘Aucune visite trouvée.’, erro: ‘Erreur de chargement.’,
explorar: ‘Explorer’, secao_label: ‘Visites’, secao_titulo: ‘Expériences disponibles’,
sob_consulta: ‘Sur demande’, por_pessoa: ‘/ personne’, modal_eyebrow: ‘Demande de Visite’,
det_eyebrow: ‘Expérience’, lbl_data: ‘Date souhaitée’, lbl_horario: ‘Heure souhaitée’,
lbl_nome: ‘Nom complet’, lbl_tel: ‘Téléphone’, lbl_qtd: ‘Nombre de personnes’,
...
if (bg && data.foto_capa) { bg.style.backgroundImage = ‘url('’ + data.foto_capa + ‘')’; bg.style.opacity = ‘1’; }
...
var primeiraFoto = (p.fotos && p.fotos.length) ? (typeof p.fotos[0]===‘string’ ? p.fotos[0] : p.fotos[0].url||’’) : ‘’;
...
(p.fotos || []).forEach(function(f) { var s = getImageUrl(f); if (s && s !== principal) fotos.push(s); });
return fotos;
}
...
if (descEl) { descEl.innerHTML = p.descricao_completa || p.descricao || ''; descEl.style.display = descEl.innerHTML.trim() ? '' : 'none'; }
...
function fecharDetalhe() { document.getElementById(‘modalDetalhe’).classList.remove(‘open’); document.body.style.overflow = ‘’; }
...
if (av) { var lc = av.lastChild; if (lc && lc.nodeType===Node.TEXT_NODE) lc.textContent = ' '+t('aviso_wpp'); }

['res-nome','res-tel','res-data','res-horario'].forEach(function(id){ var el=document.getElementById(id); if(el) el.value=''; });
...
function fecharModalReserva() { document.getElementById(‘modalReserva’).classList.remove(‘open’); document.body.style.overflow = ‘’; passeioAtual = null; }
...
? t(‘sob_consulta’)
: ’R$ ’ + (passeioAtual.preco_por_pessoa ? Number(passeioAtual.preco||0)*qtd : Number(passeioAtual.preco||0)).toLocaleString(‘pt-BR’,{minimumFractionDigits:2});
...
var msgFn = t(‘wpp_msg’);
var msg = typeof msgFn===‘function’ ? msgFn(nome,passeioAtual.nome,qtd,dl,hor) : ‘Olá! Tenho interesse no passeio ‘+passeioAtual.nome+’ para ‘+qtd+’ pessoa(s).’;
...
if (l && tipo!==‘todos’ && l.tipo!==tipo) { lugarSelecionadoId=null; carregarMapaIframe(MAPA_GERAL_SRC); }
}
renderLugarCards();
}
...
if (!iframe) return;
if (loading) { loading.classList.remove(‘oculto’); var txt=document.getElementById(‘mapa-loading-txt’); if(txt) txt.innerText=t(‘mapa_carregando’); }
iframe.onload = function(){ if(loading) loading.classList.add(‘oculto’); };
if (iframe.src !== src) iframe.src = src; else if (loading) loading.classList.add(‘oculto’);
}
...
return { id:l.id, tipo:l.tipo, emoji:l.tipo===‘restaurante’?‘🍽’:‘🛍’,
nome:{pt:l.nome,en:l.nome,es:l.nome,fr:l.nome},
desc:{pt:l.descricao,en:l.descricao,es:l.descricao,fr:l.descricao},
estrelas:l.estrelas||‘★★★★☆’, instagram:l.instagram||’’, telefone:l.telefone||’’,
...
window.fecharDetalhe=fecharDetalhe; window.handleCardClick=handleCardClick;
window.abrirModalReservaDoDetalhe=abrirModalReservaDoDetalhe; window.voltarParaDetalhe=voltarParaDetalhe;
window.fecharModal=fecharModal; window.fecharModalReserva=fecharModalReserva;
window.confirmarReserva=confirmarReserva; window.calcularTotal=calcularTotal;
window.carrosselMover=carrosselMover; window.coverflowIr=coverflowIr;
window.detFotoMover=detFotoMover; window.detFotoIr=detFotoIr;
window.filtrarMapa=filtrarMapa; window.selecionarLugar=selecionarLugar;
window.lc360Mover=lc360Mover; window.lc360Ir=lc360Ir; window.lc360Selecionar=lc360Selecionar;
